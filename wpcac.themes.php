 ...
 * @return array
 */
function _wpcac_get_themes() {

	require_once( ABSPATH . '/wp-admin/includes/theme.php' );

	_wpcac_add_non_extend_theme_support_filter();
 ...
 * @return array
 */
function _wpcac_upgrade_theme( $theme ) {

	include_once ( ABSPATH . 'wp-admin/includes/admin.php' );

	if ( ! _wpcac_supports_theme_upgrade() )
	...
	$skin = new WPCAC_Theme_Upgrader_Skin();
 ...
 * @return bool
 */
function _wpcac_supports_theme_upgrade() {
...
function _wpcac_add_non_extend_theme_support_filter() {
	add_filter( 'pre_set_site_transient_update_themes', '_wpcac_add_non_extend_theme_support' );
}

function _wpcac_add_non_extend_theme_support( $value ) {

    foreach( $non_extend_list = _wpcac_get_non_extend_themes_data() as $key => $anon_function ) {
...
function _wpcac_get_non_extend_themes_data() {

    return array(
        'pagelines' => '_wpcac_get_pagelines_theme_data'
    );

}

function _wpcac_get_pagelines_theme_data() {
	...
	if ( $update_data && isset( $update_data['package'] ) && $update_data['package'] !== 'bad' ) {
		$update_data['Template'] = 'pagelines'; // needed in _wpcac_add_non_extend_theme_support()
		return $update_data;
	}
